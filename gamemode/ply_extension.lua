...
-- here some hooks so we can do a custom HUD_PRINTCENTER replacement for GMDM
function meta:NewPrintMessage( message )
	if( SERVER ) then
		-- set up our recepient filter
...
function meta:SetupPlayerTable()
	...
	local fmn = string.lower( self:GetModel() );
	modelname = string.Replace( string.sub( fmn, 1, string.len( fmn ) - 4 ), "models/player/", "" );
	self.Info = nil
	
	for k,v in pairs(PlayerTables) do
		if string.find(v.Model, modelname) then
			self.Info = v
		end
	end
