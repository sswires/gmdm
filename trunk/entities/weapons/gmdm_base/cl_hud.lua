...
function SWEP:DrawCrosshairHUD()
	...
	local gap = ( 10 + (self.Primary.Cone * ( 170 * (ScrH()/720) ) ) * (1/self:GetStanceAccuracyBonus()));
	...
	gap = math.Clamp( gap, 0, (ScrH()/2)-35 );
	local length = ( gap + 15 ) * 0.6

	self:DrawCrosshairBit( x - gap - length - 2, y - 1, length + 2, 3 ) -- left
