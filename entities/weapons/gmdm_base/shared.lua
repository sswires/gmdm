...
function SWEP:BulletPenetrate( bouncenum, attacker, tr, dmginfo, isplayer )
	...
	if( util.tobool( GetConVarNumber( "gmdm_bulletpenetration" ) ) == false ) then return end
	...
	if( tr.MatType == MAT_DIRT or tr.MatType == MAT_WOOD or tr.MatType == MAT_FLESH or tr.MatType == MAT_ALIENFLESH ) then -- dirt == plaster, and wood should be easier to penetrate so increase the distance
		...
		local PeneTrace = {}
		   PeneTrace.start = tr.HitPos + tr.Normal
		   PeneTrace.endpos = PeneTrace.start + PeneDir
		   PeneTrace.mask = MASK_SOLID + CONTENTS_HITBOX
		   
		   
		// Quick check. This fixes a bug with going through the world into the void
		if ( util.PointContents( PeneTrace.endpos ) != 0 ) then return false end
		
		local PeneTrace = util.TraceLine( PeneTrace ) 
		
		// This shouldn't really ever happen.
		if (!PeneTrace.StartSolid) then return false end
		
		// Bullet didn't penetrate.
		if ( PeneTrace.FractionLeftSolid == 1 ) then return false end
		
		Msg( tostring( PeneTrace.FractionLeftSolid ) .. " " .. tostring( PeneTrace.Fraction  ) .. " " .. tostring( PeneTrace.Entity ) .." --\n" );

		// Work out the exit point.
		local ExitPoint = PeneTrace.StartPos + (PeneDir * PeneTrace.FractionLeftSolid)
